...
global allestimates treat treatpvalue endowp1 endowp1pval endowp2 endowp2pval 
	   endowp3 endowp3pval resid residpval $outcome $category;
# delimit cr 
matrix restimates = J(1,12,.)
...
foreach out of numlist `outcomes' {
	foreach cat of numlist `categories'{
		...
		matrix restimates_cat`cat'_o`out' = [`treat_cat`cat'_o`out'',`pvtreat_cat`cat'_o`out'',`endp1_cat`cat'_o`out'',
		              ...
		              `endp3_cat`cat'_o`out'',`pvendp3_cat`cat'_o`out'',
...
foreach cat of numlist `categories' {
	...
	# delimit
	graph hbar endowp1 endowp1_2 endowp2 endowp2_2 endowp3 endowp3_2 resid resid_2
	...
	legend(span rows(4) cols(2) label(1  "{&Delta} ${partition1label} Levels") label(2 "if p-value < .10") 
								label(3  "{&Delta} ${partition2label} Levels") label(4 "if p-value < .10")
	                    		label(5  "{&Delta} ${partition3label} Levels") label(6 "if p-value < .10")
	                    		label(7   "{&Delta} Residual")                 label(8 "if p-value < .10") 
	                    size(small)) 
